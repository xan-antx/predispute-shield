...
def main() -> None:
    ...
    report = (
        ...
        f"in {len(ties)}, loses in {len(losses)}.** Every tie is a zero-fight cell: the "
        f"curve prices all fights out and the system degenerates to always_refund "
        f"exactly. Winning Δ ranges {min(c['delta'] for c in wins):+,.0f} to "
        f"{max(c['delta'] for c in wins):+,.0f}; the worst loss is "
